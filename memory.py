...
class MemoryModule(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.shrink_thres > 0:
            ...
            # re-normalize
            att_weight = F.normalize(att_weight, dim=1)  # [N, M]

        # generate code z'
        mem_T = self.weight.permute(1, 0)
        output = F.linear(att_weight, mem_T)  # Fea*Mem^T^T : [N, M] x [M, C] = [N, C]
        output = torch.cat((output, x_nor), dim=1)

        # output = output.view(B, H, W, C).permute(0, 3, 1, 2)  # [N,C,H,W]
        output = output.view(B, H, W, C*2).permute(0, 3, 1, 2)  # [N,C,H,W]
        return att_weight, output, top1_loss, gathering_loss    #4 4 64